      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
            <div className="text-xs text-slate-400 uppercase tracking-wide mb-1">Tracking ID</div>
            <div className="font-mono text-xl font-bold text-orange-400">{submitted.trackingId}</div>
          ...
          <div className="flex flex-col gap-3">
            {submitted.pdfUrl && (
              <a
                ...
                className="w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white py-3 rounded-xl text-sm font-medium transition-colors"
              >
            ...
            <Link
              ...
              className="w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white py-3 rounded-xl text-sm font-medium transition-colors block"
            >
              🔍 Track This Report
            </Link>
            <Link
              to="/"
              className="w-full border border-slate-700 hover:border-slate-500 text-slate-400 hover:text-slate-200 py-3 rounded-xl text-sm transition-colors"
            >
              Back to Home
            </Link>
          </div>
    ...
    <div className="min-h-screen">
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">Complete in under 3 minutes</p>
        </div>
          ...
          {STEPS.map((s, idx) => (
            <div key={s.id} className="flex items-center flex-1">
              <div className="flex flex-col items-center gap-1 flex-1">
                ...
                <span className={`text-xs ${step >= s.id ? 'text-slate-300' : 'text-slate-600'}`}>
                  {s.label}
                </span>
              </div>
              {idx < STEPS.length - 1 && (
                <div className={`h-px flex-1 mx-2 mb-5 transition-colors ${step > s.id ? 'bg-green-500' : 'bg-slate-700'}`} />
              )}
            </div>
          ))}
        </div>
